/*
 GLmol - Molecular Viewer on WebGL/Javascript (0.46) - forked and modified by Mike Tyka @ Google
  ...
  Contributors:
    ...
    Mike Tyka (mtyka@google.com), selecting atoms/residues, color by element and other mods
...
// Reference: A. Bondi, J. Phys. Chem., 1964, 68, 441.
   ...
   this.cylinderRadius = 0.23;
   this.lineWidth = 3.0 * this.aaScale;
   this.curveWidth = 30 * this.aaScale;
...
GLmol.prototype.parsePDB2 = function(str) {
   ...
   for (var i = 0; i < lines.length; i++) {
      ...
      if (recordName == 'ATOM  ' || recordName == 'HETATM') {
         ...
         element = atomname_map[ elem ];
         if (line[0] == 'H') hetflag = true;
         else hetflag = false;
         atoms[serial] = {'resn': resn, 'x': x, 'y': y, 'z': z, 'elem': elem, 'element' : element,
                          'hetflag': hetflag, 'chain': chain, 'resi': resi, 'serial': serial, 'atom': atom, 'selected': false,
...
// described in CONECT. But what about 2JYT???
   ...
   for (i = 0; i < atoms.length; i++) {
      atom = atoms[i]; if (atom == undefined) continue;
      if( !( atom.chain in this.world) ) this.world[ atom.chain ] = { "name":  atom.chain, "residues" : {} }
...
//      if( !( atom.atom  in this.world[ chainname ]["residues"][ atom.resi ]["atoms"] ) ) this.world[ chainname ]["residues"][ atom.resi ]["atoms"][ atom.atom ] = atom
  ...
  data = []
  for ( var chain in this.world ){
    new_chain =  { "data" : "Chain: " + chain, "metadata" : { "chain" : chain }, "children": [] }
    for ( var resi in this.world[chain]["residues"] ){
      new_residue = { "data" : resi + ":" +  this.world[chain]["residues"][resi]["name"] , "metadata" : { "chain": chain, "resi" : resi }, "children": [] }
      for ( var iatom in this.world[chain]["residues"][resi]["atoms"] ){
        the_atom = this.world[chain]["residues"][resi]["atoms"][iatom]
        new_residue[ "children" ].push( {
          "data" : this.world[chain]["residues"][resi]["atoms"][iatom].atom ,
          "metadata" : { "serial": the_atom.serial },
//          "children": [
//                     { "data" : "atom:"  + the_atom.atom  },
//                     { "data" : "chain:" + the_atom.chain  },
//                     { "data" : "elem:"  + the_atom.elem  },
//                     { "data" : "resi:"  + the_atom.resi  },
//                     { "data" : "resn:"  + the_atom.resn  },
//                     { "data" : "serial:"+ the_atom.serial  },
//                     { "data" : "ss:"    + the_atom.ss  },
//                     { "data" : "x:"     + the_atom.x  },
//                     { "data" : "y:"     + the_atom.y  },
//                     { "data" : "z:"     + the_atom.z  },
//            ]
      ...
      new_chain[ "children" ].push( new_residue )
    }
    data.push( new_chain )
  }
  ...
  //     var selected_nodes = $.jstree._focused().get_selected();
  //
  //     for (i = 0; i < atoms.length; i++) {
  //       atom = atoms[i]; if (atom == undefined) continue;
  //       atom.selected = false;
  //     }
  //     selected_nodes.each( function( i,x ){
  //       chain = $(x).data("chain")
  //       resi = $(x).data("resi")
  //       serial = $(x).data("serial")
  //       if( chain !== undefined ){
  //         if( resi !== undefined ){
  //           for( var  iatom in world[chain]["residues"][resi]["atoms"] ) {
  //             world[chain]["residues"][resi]["atoms"][iatom].selected = true;
  //           }
  //         }else{
  //           for ( var resi in world[chain]["residues"] ){
  //             for( var  iatom in world[chain]["residues"][resi]["atoms"] ) {
  //               world[chain]["residues"][resi]["atoms"][iatom].selected = true;
  //             }
  //           }
...
GLmol.prototype.select_atoms = function( chain, resi, serial ){
  if( chain !== undefined ){
    if( resi !== undefined ){
      for( var  iatom in world[chain]["residues"][resi]["atoms"] ) {
        world[chain]["residues"][resi]["atoms"][iatom].selected = !world[chain]["residues"][resi]["atoms"][iatom].selected;
      }
    }else{
      for ( var resi in world[chain]["residues"] ){
        for( var  iatom in world[chain]["residues"][resi]["atoms"] ) {
...
GLmol.prototype.drawAtomsAsSphere = function(group, atomlist, defaultRadius, forceDefault, scale) {
   ...
   for (var i = 0; i < atomlist.length; i++) {
      ...
      sphere ["atomserial"] = atom.serial;

      if( atom.selected ){
        var sphereMaterial = new THREE.MeshBasicMaterial({color: 0xFFFFFF, shading: THREE.NoShading, side: THREE.BackSide });
        var sphere = new THREE.Mesh(sphereGeometry, sphereMaterial);
        //sphere.flipSided = true;
        group.add(sphere);
        var r = (!forceDefault && this.vdwRadii[atom.elem] != undefined) ? this.vdwRadii[atom.elem] : defaultRadius;
        if (!forceDefault && scale) r *= scale;
        r += this.selector_thickness;
        sphere.scale.x = sphere.scale.y = sphere.scale.z = r;
        sphere.position.x = atom.x;
        sphere.position.y = atom.y;
        sphere.position.z = atom.z;
...
GLmol.prototype.drawBondAsStickSub = function(group, atom1, atom2, bondR, order) {
   ...
   var p1color = atom1.color
   var p2color = atom2.color
   var mp = p1.clone().addSelf(p2).multiplyScalar(0.5);

   var c1 = new TCo(p1color), c2 = new TCo(p2color);
   if (order == 1 || order == 3) {

      this.drawCylinder(group, p1, mp, bondR, p1color, atom1.serial);
      this.drawCylinder(group, p2, mp, bondR, p2color, atom2.serial);
      if ( atom1.selected && atom2.selected ){
        this.drawCylinder(group, p1, p2, bondR+this.selector_thickness, p2color, atom1.serial, false, true);
      } else
      if ( atom1.selected ){
        this.drawCylinder(group, p1, mp, bondR+this.selector_thickness, p2color, atom1.serial, false, true);
      } else
      if ( atom2.selected ){
        this.drawCylinder(group, p2, mp, bondR+this.selector_thickness, p2color, atom2.serial, false, true);
      }
   }
   if (order > 1) {
      tmp = mp.clone().addSelf(delta);
      this.drawCylinder(group, p1.clone().addSelf(delta), tmp, bondR, p1color);
      this.drawCylinder(group, p2.clone().addSelf(delta), tmp, bondR, p2color);
      tmp = mp.clone().subSelf(delta);
      this.drawCylinder(group, p1.clone().subSelf(delta), tmp, bondR, p1color);
      this.drawCylinder(group, p2.clone().subSelf(delta), tmp, bondR, p2color);
   }
};
...
GLmol.prototype.drawBondsAsStick = function(group, atomlist, bondR, atomR, ignoreNonbonded, multipleBonds, scale) {
   ...
   for (var _i = 0; _i < nAtoms; _i++) {
      ...
      for (var _j = _i + 1; _j < _i + 30 && _j < nAtoms; _j++) {
         ...
         // dont draw bondsbetween hdyrogens even if they're v close together
         if( (atom1.element == 'H') && (atom2.element == 'H') ) { } else {
...
GLmol.prototype.drawCylinder = function(group, from, to, radius, color, atomserial, cap, selected) {
   if (!from || !to) return;
   ...
   if( selected !== undefined ) {
     var cylinderMaterial = new THREE.MeshBasicMaterial({color: 0xFFFFFF, shading: THREE.NoShading, side: THREE.BackSide  });
   }else{
   ...
   cylinder["atomserial"] = atomserial;
...
GLmol.prototype.excludeElements = function(atomlist, deleteList ) {
   var ret = [];
   var blackList = new Object();
   for (var _i in deleteList) blackList[deleteList[_i]] = true;
   for (var i in atomlist) {
...
GLmol.prototype.getSelected = function(atomlist) {
   var ret = [];
   for (var i in atomlist) {
      var atom = this.atoms[atomlist[i]]; if (atom == undefined) continue;
...
GLmol.prototype.colorByAtomElement = function(atomlist, colors) {
   for (var i in atomlist) {
      var atom = this.atoms[atomlist[i]]; if (atom == undefined) continue;
      var c = colors[atom.element ];
      if (c == undefined) c = this.defaultColor;
      atom.color = c;
   }
};
...
GLmol.prototype.enableMouse = function() {
   ...
   glDOM.click(function(ev ) {
     ...
     if( ( x == me.mouseStartX ) && ( y == me.mouseStartY ) ){
        scenex =   ( x / $(this).innerWidth() ) * 2 - 1;
        sceney = - ( y / $(this).innerHeight() ) * 2 + 1;

        var vector = new THREE.Vector3( scenex, sceney, 0.5 );
        projector = new THREE.Projector();
        projector.unprojectVector( vector, me.camera );
        var ray = new THREE.Ray( me.camera.position, vector.subSelf( me.camera.position ).normalize() );
        // yuck - woks but hacky - should nt use __objects. Find the actual array of objects, ok ?

        var intersects = ray.intersectObjects( me.scene.__objects );
        if ( intersects.length > 0 ) {
          //intersects[ 0 ].object.material.color.setHex(  0x000000 );
          var serial = intersects[ 0 ].object["atomserial"]
          console.log ( me.atoms[serial] )
          me.select_atoms( me.atoms[serial].chain, me.atoms[serial].resi, undefined )
          updateRepresentation();
        } else {
          me.unselect_all();
          updateRepresentation();
