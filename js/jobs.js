...
function prepareData(data) {
  ...
  for (var i = 0; i < data.length; i++) {
    ...
    var new_obj = {
      text: info.short,
      tags: [{
        markup:"<span class='badge'></span>",
        value: data[i].replication
      },
      {
        markup: "<span class='badge alert-success'></span>",
        value: data[i].count_results
      },
      {
        markup:"<span class='badge alert-danger'></span>",
        value: data[i].count_errors
      }],
...
$("#btn_purge_jobs").click(function() {
  $.post("/operation/deleteall", function(data) {
    updateOperationTree();
  });
});
